...
public class key_surface_del_R : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            for (int xp = ((weight_point - 1) * key_size + (weight_point - 1) * key_space) + (high_point - 1); xp < (weight_point * key_size + (weight_point - 1) * key_space) + (high_point - 1) + 3; xp++) //for (int xp = 0; xp < size - 1; xp++)
            {
